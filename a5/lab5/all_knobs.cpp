...
all_knobs_c::all_knobs_c() {
	...
	KNOB_BPRED_TYPE       = new KnobTemplate< unsigned > ("bpred_type", 4);
	...
	/* lab5 additional knobs */

	KNOB_PERCEP_HIS_LENGTH = new KnobTemplate< unsigned > ("perceptron_hist_len", 20); 
	KNOB_PERCEP_TABLE_ENTRY = new KnobTemplate< unsigned > ("perceptron_table_entry", 512); 
	KNOB_PERCEP_CTR_BITS = new KnobTemplate< unsigned > ("perceptron_ctr_bits", 8); 
	KNOB_PERCEP_THRESH_OVD = new KnobTemplate< unsigned > ("perceptron_threshold_ovd", 0); 
	
	KNOB_CLOCK_FREQ = new KnobTemplate< unsigned > ("clock_freq", 1200);
}
...
all_knobs_c::~all_knobs_c() {
	...
	delete KNOB_PERCEP_HIS_LENGTH;
	delete KNOB_PERCEP_THRESH_OVD;
	delete KNOB_PERCEP_CTR_BITS;
	delete KNOB_PERCEP_TABLE_ENTRY;
	delete KNOB_CLOCK_FREQ;
}
...
void all_knobs_c::registerKnobs(KnobsContainer *container) {
	...
	container->insertKnob( KNOB_PERCEP_HIS_LENGTH);
	container->insertKnob( KNOB_PERCEP_THRESH_OVD);
	container->insertKnob( KNOB_PERCEP_CTR_BITS);
	container->insertKnob( KNOB_PERCEP_TABLE_ENTRY);
	container->insertKnob( KNOB_CLOCK_FREQ);
...
void all_knobs_c::display() {
	...
	KNOB_PERCEP_HIS_LENGTH->display(cout); cout << endl;
	KNOB_PERCEP_THRESH_OVD->display(cout); cout << endl;
	KNOB_PERCEP_CTR_BITS->display(cout); cout << endl;
	KNOB_PERCEP_TABLE_ENTRY->display(cout); cout << endl;
	KNOB_CLOCK_FREQ->display(cout); cout << endl;
