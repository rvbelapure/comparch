...
typedef struct Cache_Entry_Struct{
  ...
  uint64_t lru;        /* for replacement policy */
  int valid;      /* valid bit */
...
void cache_update(Cache * cache, ADDRINT addr);
int get_repl_index(Cache *cache, int index); 

extern uint64_t dcache_reads,dcache_writes;

/* local variables for D-cache   */

// initial parameter 
static int cache_size, block_size, assoc;
static int dcache_latency, memory_latency, bus_width;
