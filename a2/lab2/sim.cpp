...
//			cache_insert(data_cache,op->st_vaddr);
#if 0
		if(op->mem_type == MEM_ST)
		{
			/* if it is store, add it to MSHR then retire */
			if(main_memory->insert_mshr(op))
			{
				/* added successfully into mshr */
//				MEM_latch->op = op;		// deprecated
...
//			if(op->mem_type == MEM_ST)
//			{
				/* if this is a store, allow it to retire */
//				MEM_latch->op = op;		// deprecated
//				MEM_latch->oplist.push_back(op);
//				MEM_latch->op_valid = true;
//			}
...
void ID_stage()
{
	...
	if((op->cf_type >= CF_BR) && (op->cf_type < NUM_CF_TYPES))
	{
		/* A branch instruction is identified. Tell the FE stage to stall.
		   Control Hazard.*/
