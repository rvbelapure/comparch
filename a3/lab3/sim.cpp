...
/* There is error in new get_op(). Uncomment this macro to enable new get_op() function
   once the error is resolved */
#define USE_NEW_GETOP
...
bool get_op(Op *op)
{
  ...
  if (success) {
    ...
    op->valid    = TRUE;
//    op->mem_decrement = true;			/* From instructor's implementation of Lab2 */
  }
  return success;
}
#else
...
void init_structures(memory_c *main_memory) // please modify init_structures function argument  /** NEW-LAB2 */ 
{
  ...
  if(vmem_enabled)
	  dtlb = tlb_new(tlb_size);		// XXX check unit of tlb size, otherwise multiply by appropriate factor
  else
	  dtlb = NULL;
}
...
//  if(last_instruction_recvd && (main_memory->m_mshr.empty()) && (!MEM_latch->op_valid))
  ...
  for(list<Op *>::iterator it = MEM_latch->oplist.begin() ; it != MEM_latch->oplist.end() ; it++)
  {
	  ...
	  if((op->cf_type >= CF_BR) && (op->cf_type < NUM_CF_TYPES) && (FE_latch->stall_enforcer == op->inst_id))
	  {
...
void MEM_stage(memory_c *main_memory)  // please modify MEM_stage function argument  /** NEW-LAB2 */ 
{
	...
	Op *op = EX_latch->op;
	int threadid = 0;		/* keep this 0 for LAB 3 */
...
void ID_stage()
{
	...
	if((!use_bpred) && (op->cf_type >= CF_BR) && (op->cf_type < NUM_CF_TYPES))
	{
		...
		FE_latch->stall_enforcer = op->inst_id;
...
{                                          // NEW-LAB2 
	...
	if(op->is_pteop)
	{
		uint64_t pfn = vmem_vpn_to_pfn(op->vpn, op->threadid);
		tlb_install(dtlb,op->vpn,op->threadid,pfn);
		free_op(op);
		pteop_returning_on_mshr_full = false;
		EX_latch->pipeline_stall_enabled = false;
